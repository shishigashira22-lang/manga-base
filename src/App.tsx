...
const SAMPLE_MANGA: Manga[] = [
  ...
  { id: 5, title: "チェンソーマン", author: "藤本タツキ", genre: ["ダーク","アクション"], rating: 4.7, reviews: 2187, cover: "https://picsum.photos/seed/chainsaw/200/280", description: "悪魔と融合した青年デンジの激烈な生存記。", price: "528円/巻", publisher: "集英社", year: 2018, rank: 5 },
...
function MangaCard({ manga, onClick, rank }: { manga: Manga; onClick: (m: Manga) => void; rank: number | null }) {
  ...
  return (
    ...
    >
      ...
      <img src={manga.cover} alt={manga.title} style={{ width: "100%", height: 200, objectFit: "cover", display: "block" }} />
      <div style={{ padding: "12px 14px", flex: 1, display: "flex", flexDirection: "column", gap: 6 }}>
        <div style={{ fontWeight: 800, fontSize: "0.95rem", color: "#f0f0f0", lineHeight: 1.3 }}>{manga.title}</div>
        <div style={{ fontSize: "0.78rem", color: "#999" }}>{manga.author}</div>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 4, marginTop: 2 }}>
          {manga.genre.map((g: string) => (
            <span key={g} style={{ background: "#1e1e35", color: "#a0a0c0", fontSize: "0.68rem", padding: "2px 7px", borderRadius: 10 }}>{g}</span>
          ))}
        </div>
    ...
    <div onClick={onClose} style={{
      ...
      <div onClick={(e) => e.stopPropagation()} style={{
        ...
        <div style={{ display: "flex", gap: 20, padding: "24px 24px 0" }}>
          <img src={manga.cover} alt={manga.title} style={{ width: 130, height: 185, objectFit: "cover", borderRadius: 8, flexShrink: 0 }} />
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: "1.35rem", fontWeight: 900, color: "#f0f0f0", lineHeight: 1.3, marginBottom: 6 }}>{manga.title}</div>
            ...
            <div style={{ marginTop: 14 }}>
              <span style={{ background: "#e63946", color: "#fff", padding: "6px 18px", borderRadius: 8, fontWeight: 700, fontSize: "0.85rem", cursor: "pointer" }}>
                📖 楽天ブックスで購入
              </span>
            </div>
          </div>
            ...
            {submitted ? (
              ...
              <>
                ...
                <button onClick={() => setSubmitted(true)}
                  style={{ marginTop: 10, background: "#e63946", color: "#fff", border: "none", borderRadius: 8, padding: "8px 22px", fontWeight: 700, cursor: "pointer", fontSize: "0.85rem" }}>送信</button>
              </>
            )}
          </div>
        </div>
        <button onClick={onClose} style={{ position: "absolute", top: 14, right: 18, background: "none", border: "none", color: "#666", fontSize: "1.4rem", cursor: "pointer" }}>✕</button>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  const fetchRecommend = async () => {
    ...
    try {
      ...
      const text = data.content?.find((b: { type: string; text?: string }) => b.type === "text")?.text || "結果を取得できませんでした。";
      setAiResult(text);
    } catch {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0d0d18", color: "#e0e0f0", fontFamily: "'Noto Sans JP', sans-serif" }}>
      ...
      {/* Header */}
      <header style={{ background: "rgba(13,13,24,0.95)", borderBottom: "1px solid #1e1e35", position: "sticky", top: 0, zIndex: 50, backdropFilter: "blur(12px)" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto", padding: "0 20px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 60 }}>
          <div style={{ fontFamily: "'Zen Maru Gothic', serif", fontWeight: 900, fontSize: "1.4rem", color: "#e63946" }}>📚 マンガベース</div>
          <div style={{ fontSize: "0.78rem", color: "#555" }}>漫画検索・ランキング・レビューサイト</div>
        </div>
        <div style={{ maxWidth: 1100, margin: "0 auto", padding: "0 20px", display: "flex", borderTop: "1px solid #1e1e35" }}>
      ...
      <main style={{ maxWidth: 1100, margin: "0 auto", padding: "30px 20px" }}>
        ...
        {(tab === "ranking" || tab === "search") && (
          <>
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 28 }}>
              {tab === "search" && (
                <input ref={inputRef} value={query} onChange={(e) => setQuery(e.target.value)}
                  ...
                  style={{ flex: "1 1 240px", background: "#13131f", border: "1px solid #2a2a3e", borderRadius: 8, color: "#e0e0f0", padding: "10px 16px", fontSize: "0.9rem" }} />
              )}
            ...
            {tab === "ranking" && genre === "すべて" && (
              <div style={{ marginBottom: 32 }}>
                <div style={{ fontWeight: 900, fontSize: "1.1rem", color: "#e63946", marginBottom: 16 }}>🔥 TOP 3</div>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 16 }}>
                  {filtered.slice(0, 3).map((m) => (
                    <div key={m.id} onClick={() => setSelected(m)}
                      style={{ background: "linear-gradient(135deg, #1a1a2e 0%, #16213e 100%)", border: "1px solid #e63946", borderRadius: 12, padding: "16px 18px", cursor: "pointer", display: "flex", gap: 14, alignItems: "center", transition: "transform 0.2s" }}
                      onMouseEnter={(e) => (e.currentTarget.style.transform = "scale(1.02)")}
                      onMouseLeave={(e) => (e.currentTarget.style.transform = "scale(1)")}>
                      <div style={{ fontSize: "2rem", fontWeight: 900, color: "#e63946", minWidth: 40 }}>#{m.rank}</div>
                      <img src={m.cover} alt={m.title} style={{ width: 56, height: 78, objectFit: "cover", borderRadius: 6 }} />
                      <div>
                        <div style={{ fontWeight: 800, fontSize: "1rem", color: "#f0f0f0" }}>{m.title}</div>
            ...
            <div style={{ color: "#888", fontSize: "0.85rem", marginBottom: 14 }}>{filtered.length}件表示</div>
            <div className="card-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(170px, 1fr))", gap: 18 }}>
              {filtered.map((m) => (
                <MangaCard key={m.id} manga={m} onClick={setSelected} rank={sortBy === "rank" ? m.rank : null} />
              ))}
            </div>
            {filtered.length === 0 && (
              <div style={{ textAlign: "center", color: "#555", padding: "60px 0", fontSize: "0.95rem" }}>該当する漫画が見つかりませんでした。</div>
            )}
          </>
        )}
        ...
        {tab === "recommend" && (
          <div style={{ maxWidth: 680, margin: "0 auto" }}>
            <div style={{ textAlign: "center", marginBottom: 32 }}>
              <div style={{ fontWeight: 900, fontSize: "1.6rem", color: "#f0f0f0", marginBottom: 8 }}>🤖 AI漫画おすすめ</div>
              <div style={{ color: "#888", fontSize: "0.9rem" }}>好みや気分を入力すると、AIが漫画をおすすめします</div>
            </div>
            <div style={{ display: "flex", gap: 8 }}>
              <input value={aiRec} onChange={(e) => setAiRec(e.target.value)}
              ...
              <button onClick={fetchRecommend}
                style={{ background: "#e63946", color: "#fff", border: "none", borderRadius: 10, padding: "12px 22px", fontWeight: 700, cursor: "pointer", fontSize: "0.9rem" }}>検索</button>
            </div>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 14 }}>
              {["泣ける名作","異世界転生系","スポーツ漫画","ミステリー・サスペンス","子供と一緒に読める"].map((ex) => (
                <span key={ex} onClick={() => setAiRec(ex)}
                  style={{ background: "#1a1a2e", border: "1px solid #2a2a3e", color: "#a0a0c0", padding: "5px 12px", borderRadius: 20, fontSize: "0.78rem", cursor: "pointer" }}>{ex}</span>
              ))}
            </div>
            {aiLoading && (
              <div style={{ textAlign: "center", padding: "40px 0", color: "#888" }}>
                <div style={{ fontSize: "2rem", marginBottom: 8 }}>⏳</div>AIが考え中...
              </div>
            )}
            {aiResult && !aiLoading && (
              <div style={{ marginTop: 28, background: "#13131f", border: "1px solid #2a2a3e", borderRadius: 14, padding: "22px 24px" }}>
                <div style={{ fontWeight: 700, color: "#e63946", marginBottom: 12, fontSize: "0.85rem" }}>🤖 AIのおすすめ</div>
                <div style={{ color: "#d0d0e0", lineHeight: 1.9, fontSize: "0.9rem", whiteSpace: "pre-wrap" }}>{aiResult}</div>
              </div>
            )}
          </div>
        )}
      ...
      <footer style={{ borderTop: "1px solid #1e1e35", padding: "24px 20px", textAlign: "center", color: "#444", fontSize: "0.78rem", marginTop: 60 }}>
        © 2026 マンガベース ／ データ提供: 楽天ブックスAPI（連携済み）
      </footer>
