...
async function fetchManga(keyword: string, sort: string = "sales"): Promise<Manga[]> {
  try {
    ...
    const params = isGenreId
      ? "genreId=" + keyword + "&sort=" + encodeURIComponent(sort)
      : "keyword=" + encodeURIComponent(keyword) + "&sort=" + encodeURIComponent(sort);
...
export default function App() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0d0d18", color: "#e0e0f0", fontFamily: "'Noto Sans JP', sans-serif" }}>
      ...
      {/* Header */}
      <header style={{ background: "rgba(10,10,20,0.97)", borderBottom: "1px solid #1a1a30", position: "sticky", top: 0, zIndex: 10, backdropFilter: "blur(16px)" }}>
